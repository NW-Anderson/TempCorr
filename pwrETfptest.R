...
for(j in 1:length(ntaxa)){
  ...
  for(i in 1:length(oddrate)){
    ...
    sig.tests <- foreach(h = 1:smp.size, .options.multicore=opts, .combine = 'c') %dopar% {
      ...
      tree <- trees(pars=c(1,.1), type="bd", max.taxa=ntaxa[j])[[1]]
      tree$edge.length <- tree$edge.length/max(branching.times(tree))
      # creating simulated tip state and organizing into a data frame
      par <- rbind(c(0, .1, .5, 0), c(.1, 0, 0 ,.5), c(.5, 0, 0, oddrate[i]), c(0, .5, .1, 0))
      ...
      while(good.data2 == F){ 
        ...
        while(good.data1 == F){
          ...
          if(sum(data1 == 1)>.3*ntaxa[j] && sum(data1 == 1)<.7*ntaxa[j] &&
             sum(data2 == 1)>.3*ntaxa[j] && sum(data2 == 1)<.7*ntaxa[j]) good.data1 <- T
        }
